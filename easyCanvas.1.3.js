...
function easyCanvas() {
	...
	var me = this, w, h, el, i = 0, ctx,
		...
		_clearHack = testClear();	// test for which clear method to use. ver 1.3
	...
	*/
	...
	this.crossOrigin		= false;	// ver. 1.3
	this.crossOriginMode	= 'anonymous';
	...
	*/
	...
	this.resize = function(w, h) {
		...
		try {
			...
			if (ctx !== undefined) {
				calcDeltas();
				setTransform();
				this.antialiasing(this.antialiasing()); // added in 1.3
			}
	...
	// get grid snapshot (v1.3)
	this.getGridSnapshot = function(maxWidth, maxHeight) {

		if (gridX === 0 && gridY === 0)
			return null;

		var cols = [], rows = [],
			x = gOffsetX, y = gOffsetY;
	...
	this.loadImage = function(url, callback, callbackError) {
		...
		if (this.crossOrigin === true)
			img.crossOrigin = this.crossOriginMode;
	...
	*/
	...
	this.ge = function(type) {
		return document.querySelector(type) || document.getElementById(type);
	};
	...
	*/
	...
	// ver 1.3 - test to determine which clear methid to use, return true for width hack
	function testClear() {

		// http://jsperf.com/canvas-clear-versus-create

		// in Chrome, width hack is fastest - check if still valid
		if (window.chrome) {
			...
			// fill and reset
			tx.fillRect(0, 0, 32, 1);
			tc.width = 8;

			// pixel got cleared?
			px = tx.getImageData(0, 0, 1, 1).data;

			return (px[3] === 0);

		}

		// clearRect is fastest in FF
		return false;
	}
...
window.requestAnimationFrame = window.requestAnimationFrame ||
							   ...
							   function(cb){window.setTimeout(cb, 15)};
